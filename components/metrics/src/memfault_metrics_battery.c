...
static void prv_accumulate_discharge_session_drop(bool session_valid,
  ...
  if (session_valid && (drop >= 0)) {
    ...
    MEMFAULT_HEARTBEAT_SET_UNSIGNED(battery_soc_pct_drop, (uint32_t)drop);

    // store the session length
    MEMFAULT_HEARTBEAT_SET_UNSIGNED(battery_discharge_duration_ms, session_length_ms);
  }
}
...
void memfault_metrics_battery_collect_data(void) {
  ...
  MEMFAULT_HEARTBEAT_SET_UNSIGNED(battery_soc_pct, (uint32_t)current_stateofcharge);
}
